...
from process_dataset import generate_artifical_injected, import_compas_score_processed, import_law_processed, import_german_processed, import_compas_scores, import_law_eth
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(description='Sensitivity analysis')
    ...
    parser.add_argument('--output_dir', 
        ...
        default="Z_divrank_results_sensitivity", 
        required=False,
        type=str)
    ...
    if dataset_name == 'law_school-poc':
        ...
        data, target_name, sensitive_attributes = import_german_processed(use_foreigner=True)
    ...
    protected_attributes_list = [sensitive_attributes]
    ...
    for min_sup in [0.001, 0.005, 0.01, 0.1, 0.2, 0.3]:
        ...
        for selected_attributes in protected_attributes_list:
            ...
            config = f'{mitigation_name} - {target} - {min_sup} : {", ".join(selected_attributes)}'

            input_configuration = {'method': mitigation_name, 'protected attributes': f'{", ".join(sorted(selected_attributes))}', 'protected values': '-'}
            ...
            assert len([i for i in range(0, len(all_results)-1) if all_results['min_div'].values[i]>all_results['min_div'].values[i+1]])==0, 'Monotonicity not satisfied'
            ...
            ranking_results[f'original - {config}'] = divergence_stats(df_original, target, min_sup=min_sup)
    ...
    with open(os.path.join(output_dir, f'{dataset_name}_divrank_results_sensitivity.pickle'), 'wb') as fp:

        pickle.dump(ranking_results, fp)

    with open(os.path.join(output_dir, f'{dataset_name}_divrank_results_sensitivity_time.pickle'), 'wb') as fp:

        pickle.dump(divrank_time, fp)


#python A_004_divrank_sensitivity.py --dataset_name law_school
#python A_004_divrank_sensitivity.py --dataset_name german
#python A_004_divrank_sensitivity.py --dataset_name artificial_1
#python A_004_divrank_sensitivity.py --dataset_name compas-poc