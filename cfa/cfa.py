"""""" """"
The code of Continuous Fairness Algorithm from https://github.com/MilkaLichtblau/ContinuousFairness/blob/master/src/cfa/cfa.py
Author: Meike Zehlike 

Modification by ANONYMOUS-AUTHOR to handle package updates (e.g., deprecation warning)
are declared
""" """"""
...
############################################
# #ANONYMOUS-AUTHOR - MODIFIED to not show pandas deprecation warnings
import warnings
...
class ContinuousFairnessAlgorithm:
    ...
    def __init__(
        ...
        self.__rawDataByGroup = self._getScoresByGroup(
            self.__rawData, self.__qualityAtribute
        )
        ...
        self.__bin_edges = np.arange(
            rawData[qual_attr].min() - score_stepsize,
            rawData[qual_attr].max() + score_stepsize,
            score_stepsize,
        )
        ...
        self.__groupNamesForPlots = self.__rawDataByGroup.rename(
            group_names, axis="columns"
        )
    ...
    def _getScoresByGroup(self, dataset, scoreColName):
        ...
        for _, group in self.__groups.iterrows():
            ...
            resultCol = pd.DataFrame(data=copy[scoreColName].values, columns=[colName])
    ...
    def _dataToHistograms(self, data, bin_edges):
        ...
        for colName in data.columns:
            ...
            colAsHist = np.histogram(colNoNans[colName], bins=bin_edges, density=True)[
                0
            ]
    ...
    def _getTotalBarycenter(self, group_histograms_raw):
        ...
        total_bary = ot.bregman.barycenter(
        ...
        if self.__plot:
            self.__plott(
                pd.DataFrame(total_bary), "totalBarycenter.png", xLabel="raw score"
            )
    ...
    def _get_group_barycenters(self, total_bary, group_histograms):
        ...
        for groupName in group_histograms:
            ...
            groupMatrix = pd.concat(
                [group_histograms[groupName], pd.Series(total_bary)], axis=1
            )
            # get corresponding theta
            theta = self.__thetas[group_histograms.columns.get_loc(groupName)]
            # calculate barycenters
            weights = np.array([1 - theta, theta])
            group_barycenters[groupName] = ot.bregman.barycenter(
                groupMatrix.values,  # ANONYMOUS-AUTHOR - MODIFIED for new OT version
                self.__lossMatrix,
                self.__regForOT,
                weights=weights,
                verbose=True,
        ...
        if self.__plot:
            self.__plott(group_barycenters, "groupBarycenters.png", xLabel="raw score")
        return group_barycenters

    def _calculateFairReplacementStrategy(
        self, group_barycenters, group_histograms_raw
    ):
        ...
        """
        # print(type(group_barycenters), type(group_histograms_raw), type(self.__lossMatrix))
        ...
        for groupName in self.__groupColumnNames:
            ...
            if (
                group_histograms_raw[groupName].shape
                != group_barycenters[groupName].shape
            ):
                raise ValueError(
            ...
            ot_matrix = ot.emd(
                group_histograms_raw[groupName].values.astype(
                    np.float64
                ),  # ANONYMOUS-AUTHOR - MODIFIED for new OT version
                group_barycenters[groupName].values.astype(
                    np.float64
                ),  # ANONYMOUS-AUTHOR - MODIFIED for new OT version
                self.__lossMatrix,
            )
            ...
            groupFairScores[groupName] = np.matmul(
                normalized_ot_matrix, self.__bin_edges[1:].T
            )
    ...
    def _replaceRawByFairScores(self, groupFairScores):
        ...
        def replace(rawData, oldValue, newValue):
            ...
            for index, fairScore in fairScores.items():
                ...
                rawData.loc[
                    replaceAtIndex, newValue
                ] = fairScore  ##ANONYMOUS-AUTHOR - MODIFIED for new OT version
                # rawData.at[replaceAtIndex, newValue] = fairScore
            return rawData

        self.__rawData = self.__rawData.groupby(
            list(self.__groups.columns.values), as_index=False, sort=False
        ).apply(replace, oldValue=self.__qualityAtribute, newValue="fairScore")
        ...
        if self.__plot:
            ...
            plt.savefig(
                self.__plotPath + "fairScoreDistributionPerGroup.png",
                dpi=100,
                bbox_inches="tight",
            )

            bin_edges = np.linspace(
                groupFairScores.min().min(),
                groupFairScores.max().max(),
                int(self.__num_bins),
            )
            group_histograms_fair = self._dataToHistograms(fairDataPerGroup, bin_edges)
            self.__plott(
                group_histograms_fair,
    ...
    def run(self):
        ...
        group_histograms_raw = self._dataToHistograms(
        ...
        total_bary = self._getTotalBarycenter(
            group_histograms_raw.values
        )  # ANONYMOUS-AUTHOR - MODIFIED for new OT version
        group_barycenters = self._get_group_barycenters(
            total_bary, group_histograms_raw
        )
